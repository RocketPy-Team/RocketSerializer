...
from open_rocket_serializer.components.open_rocket_wrangler import process_elements_position
...
def generate(bs, eng, path, ork, team_name):
    ...
    cg_location_vect = [float(datapoint.text.split(',')[data_labels.index('CG location')]) for datapoint in datapoints]
    altitude_vect = [float(datapoint.text.split(',')[data_labels.index('Altitude')]) for datapoint in datapoints]
    apogee_index = np.argmax(altitude_vect)
    mass = [float(datapoint.text.split(',')[data_labels.index('Mass')]) for datapoint in datapoints]
    
    if 'Propellant mass' in data_labels:
        propellant_mass_vect = [float(datapoint.text.split(',')[data_labels.index('Propellant mass')]) for datapoint in datapoints]
    elif 'Motor mass' in data_labels:
        motor_mass = np.array([float(datapoint.text.split(',')[data_labels.index('Motor mass')]) for datapoint in datapoints])
    ...
    normalize_propellant_mass_vect = normalize_propellant_mass_vect - normalize_propellant_mass_vect[np.argmin(normalize_propellant_mass_vect)]
    ...
    rocket_radius = ork.getRocket().getChild(0).getChild(1).getAftRadius() # Improve this

    last_element = [key for key in elements.keys()][-1]

    longitudinal_moment_of_inertia = [float(datapoint.text.split(',')[data_labels.index('Longitudinal moment of inertia')]) for datapoint in datapoints][burnout_position]
    rotational_moment_of_inertia = [float(datapoint.text.split(',')[data_labels.index('Rotational moment of inertia')]) for datapoint in datapoints][burnout_position]

    ## Motor
    motor_cm = empty_rocket_cm - (cg_location_vect[0] * mass[0] - empty_rocket_cm * rocket_mass)/propellant_mass
    ...
    thrust_source_path = eng or generate_thrust_curve(path, datapoints, data_labels, burnout_position, time_vect)
    
    flight_parameters.update(save_drag_curve(datapoints[0: apogee_index], data_labels, path))
    ...
    flight_parameters.update(search_motor(burnout_time, propellant_mass, motor_radius, motor_length))

    adittional_parameters = {
            'rocketMass': float(rocket_mass), 'elevation': 160, 'date': "2022-10-14 14:00:00",
            'emptyRocketCm': empty_rocket_cm, 'radius': float(rocket_radius), 'MotorCm': float(motor_cm), 'distanceRocketNozzle': nozzle_cm,
            'inertiaI': longitudinal_moment_of_inertia, 'inertiaZ': rotational_moment_of_inertia, 'railLength': 12,
            'inclination': 84, 'heading': 133, 'thrustSource': thrust_source_path,
            'latitude': 39.3897, 'longitude' : -8.28896388889, 'distanceRocketPropellant': float(motor_cm),
            'railButtonDistToCM1': 0.2, 'railButtonDistToCM2': -0.5, 'name': team_name
    }
    flight_parameters.update(adittional_parameters)
    print(f'[AdittionalParameters] Configuration: \n{yaml.dump(adittional_parameters, default_flow_style=False)}')
