...
def search_parachute(bs):
    ...
    for idx, main_chute in enumerate(filter(lambda x: 'Main' in x.find('name').text, chutes)):
        main_cds = 'auto' if 'auto' in main_chute.find('cd').text else float(main_chute.find('cd').text)
