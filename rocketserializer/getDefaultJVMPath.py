...
class JVMFinder:
    ...
    def __init__(self):
        # Search methods
        self._methods = (self._get_from_java_home, self._get_from_known_locations)

    def _is_valid_jvm_candidate(self, candidate):
        if not candidate or not os.path.exists(candidate):
            return False

        try:
            self.check(candidate)
        except (JVMNotSupportedException, OSError, ValueError):
            return False
        return True
    ...
    def find_libjvm(self, java_home):
        ...
        # Look for the file
        for root, _, names in sorted(
            os.walk(java_home), key=lambda t: len(t[0].split(os.sep))
        ):
    ...
    def get_jvm_path(self):
        ...
        raise JVMNotFoundException(
            "No JVM shared library file ({0}) "
            "found. Try setting up the JAVA_HOME "
            "environment variable properly.".format(self._libfile)
        )
...
class WindowsJVMFinder(JVMFinder):
    ...
    def _probe_java_binary(self, java_binary):
        if not java_binary or not os.path.exists(java_binary):
            return False
    ...
    def _discover_java_home(self, java_binary):
        if not java_binary or not os.path.exists(java_binary):
            return None
        ...
        match = re.search(r"java\.home\s*=\s*(.+)", completed.stdout)
        if not match:
            return None

        java_home = match.group(1).strip().strip('"')
        if java_home and os.path.exists(java_home):
            return java_home
    ...
    @staticmethod
    def _candidate_jvm_libraries(java_home):
        java_home = Path(java_home)
        candidates = []

        for root in (java_home, java_home / "jre", java_home / "jdk"):
            if not root.exists():
                continue

            for relative_path in (
                Path("bin") / "server" / "jvm.dll",
                Path("bin") / "jvm.dll",
                Path("lib") / "server" / "jvm.dll",
                Path("jre") / "bin" / "server" / "jvm.dll",
                Path("jre") / "bin" / "jvm.dll",
                Path("jre") / "lib" / "server" / "jvm.dll",
            ):
    ...
    def _get_from_path(self):
        java_candidates = []
        for binary_name in ("java", "java.exe"):
            resolved = shutil.which(binary_name)
            if resolved:
                java_candidates.append(resolved)

        if not java_candidates:
            return None

        for java_binary in java_candidates:
            java_binary = os.path.realpath(java_binary)
            if not self._probe_java_binary(java_binary):
                continue
            ...
            java_home = self._discover_java_home(java_binary)
            if java_home:
                java_home_candidates = [Path(java_home)]
            else:
                java_home_candidates = [
                    java_path.parent.parent,
                    java_path.parent.parent.parent,
                ]

            for candidate_root in java_home_candidates:
                if not candidate_root.exists():
                    continue

                for candidate in self._candidate_jvm_libraries(candidate_root):
                    if self._is_valid_jvm_candidate(str(candidate)):
    ...
    def _get_from_registry():
        ...
        for location in WindowsJVMFinder.reg_keys:
            for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
                try:
                    jreKey = winreg.OpenKey(hive, location)
                    try:
